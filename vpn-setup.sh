...
function copyText(t){if(navigator.clipboard&&window.isSecureContext){return navigator.clipboard.writeText(t)}const ta=document.createElement('textarea');ta.value=t;ta.style.cssText='position:fixed;top:-9999px;left:-9999px;opacity:0';document.body.appendChild(ta);ta.focus();ta.select();ta.setSelectionRange(0,ta.value.length);let ok=false;try{ok=document.execCommand('copy')}catch(e){}document.body.removeChild(ta);return ok?Promise.resolve():Promise.reject(new Error('copy failed'))}
function copyLink(){const t=document.getElementById('detail-link-text').textContent;const b=document.querySelector('#detail-link .copy-btn');copyText(t).then(()=>{b.textContent='Скопировано!';setTimeout(()=>b.textContent='Копировать',1500)}).catch(()=>{b.textContent='Не удалось — выделите вручную';setTimeout(()=>b.textContent='Копировать',2500);const r=document.createRange();r.selectNodeContents(document.getElementById('detail-link-text'));const s=window.getSelection();s.removeAllRanges();s.addRange(r)})}
